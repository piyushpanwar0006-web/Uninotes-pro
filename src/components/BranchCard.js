...
const iconMap = {
  // Original Keys (Matching branchData.js)
...
export default function BranchCard({ name, subjectsCount }) {
  const IconComponent = iconMap[name] || Building2;
  const Icon = IconComponent || (() => <div className="w-10 h-10 bg-slate-200 rounded-lg animate-pulse" />);
  ...
  return (
    ...
    >
      ...
      <div className="w-16 h-16 bg-slate-50 text-emerald-500 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-500 shadow-sm">
      ...
      <p className="text-sm font-medium text-slate-400 mb-6 uppercase tracking-widest group-hover:text-slate-500">
        PYQs & Notes Available
      </p>

      <div className="mt-auto flex items-center text-emerald-600 font-bold text-sm gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
